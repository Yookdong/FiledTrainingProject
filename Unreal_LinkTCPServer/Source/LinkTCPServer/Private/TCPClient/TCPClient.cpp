...
bool TCPClient::Connect(FSocket* Socket, FString IPAddress, int PortNumber)
{
	FIPv4Address IPAddr;
...
bool TCPClient::Receive(FSocket* Socket, uint8* Results, int32 Size)
{
...
bool TCPClient::Send(FSocket* Socket, const uint8* Buffer, int32 Size)
{
	while (Size > 0)
	{
		int32 BytesSent = 0;
...
bool TCPClient::SendPacket(FSocket* Socket, uint32 Type, const TArray<uint8>& Payload)
{
	return SendPacket(Socket, Type, Payload.GetData(), Payload.Num());
}

bool TCPClient::SendPacket(
...
bool TCPClient::ReceivePacket(FSocket* Socket, TArray<uint8>& OutPayload)
{
...
void TCPClient::Send(uint32 Type, const FString& Text)
{
	...
	if (TCPClient::SendPacket(Socket, Type, WriterArray))
	{
...
void TCPClient::Recv()
{
	...
	if (TCPClient::ReceivePacket(Socket, Payload))
	{
